// Kitchen Display - Minimal Version
...
function KitchenDisplay() {
  ...
  useEffect(() => {
        ...
        const ordersWithItems = await Promise.all(
          active.map(async order => {
            try {
              ...
              return {...order, created_at: new Date(order.created_at), items};
            } catch {
              return {...order, created_at: new Date(order.created_at), items: []};
            }
          })
    ...
    socket.on('new_kitchen_order', (data) => {
      ...
      setNotification(`🔔 New Order #${data.order_id} - Table ${data.table_id}`);
      playSound();
    ...
    socket.on('order_cancelled', (data) => {
      ...
      // Remove immediately
      setOrders(prev => {
        console.log('Current orders before filter:', prev.map(o => o.id));
        const filtered = prev.filter(o => o.id !== data.order_id);
        console.log('Orders after filter:', filtered.map(o => o.id));
        return filtered;
      });
    });
...
ReactDOM.render(<KitchenDisplay />, document.getElementById('root'));