...
app.post("/sessions/open", async (req, res) => {
  try {
    ...
    if (existing.rows.length > 0) {
      return res.json({ session: existing.rows[0], message: "Session already active" });
    }
...
app.post("/reservations", async (req, res) => {
  try {
    const { table_id = 1, customer_name, customer_phone, reservation_date, reservation_time, party_size, notes, user_id = 1 } = req.body;
    ...
    // 📱 SEND SMS (Simulated - replace with real SMS service)
    const smsMessage = `Rezervimi konfirmuar!\nRestorant XYZ\n📅 ${reservation_date} ${reservation_time}\n🔐 Kodi: ${reservationCode}\nPërdoreni në kiosk.`;
    
    console.log(`📱 SMS TO ${customer_phone}:`);
    console.log(smsMessage);
    console.log('---');
    
    // TODO: Integrate real SMS service (Twilio, etc.)
    // await sendSMS(customer_phone, smsMessage);
    
    console.log(`✅ Reservation created - Table ${table_id}, Code: ${reservationCode}`);
    res.json({ 
...
app.get("/dashboard/stats", async (req, res) => {
  try {
    ...
    const [salesRes, ordersRes, lowStockRes, topProductsRes] = await Promise.all([
      pool.query(
        "SELECT COALESCE(SUM(total_amount), 0) as total FROM table_sessions WHERE DATE(closed_at) = $1",
        [today]
      ),
      pool.query(
      ...
      pool.query("SELECT COUNT(*) as count FROM low_stock_items"),
      pool.query(`
